	...
	if file.ID == 0 {
		...
		_, err := r.db.ExecContext(ctx, `
			...
			SET 
				...
				status = $10
			WHERE id = $11`,
			file.FileID, file.Filepath, file.SaluteId, file.RecognizeTaskID, file.RecognizeStatus, file.ResponseFileID, file.Content, file.ProcessNotificationSent, file.Encoding, file.Status,
			file.ID)
		if err != nil {
