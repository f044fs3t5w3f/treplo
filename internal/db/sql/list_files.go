...
func (r *repository) ListFiles(ctx context.Context) ([]*models.File, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, chat_id, message_id, file_id, filepath, salute_id,recognize_task_id, recognize_status, response_file_id, dialogue_content, process_notification_sent, encoding, created_at FROM files
	`)
	if err != nil {
		return nil, err
	}
